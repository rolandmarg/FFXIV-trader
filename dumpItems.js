...
async function bootstrap() {
  ...
  const response = await axios.get(
    `https://xivapi.com/search?filters=LevelItem=1`
  );
  ...
  for (let i = 2; i <= total; i++) {
    await Promise.all([
      axios
        .get(`https://xivapi.com/search?filters=LevelItem=1&page=${i}`)
        .then((r) => results.push(r.data.Results)),
  ...
  let output = '';
  Object.values(prices.itemPrices)
    .sort((a, b) => b.globalPrice.pricePerUnit - a.globalPrice.pricePerUnit)
    .forEach((i) => {
      if (i.globalPrice.pricePerUnit < 10000) {
        return;
      }
      output += `${
        i.cheapest && i.cheapest.name
      }  [${i.globalPrice.pricePerUnit.toLocaleString('en-US')}G]\n`;
    });
