...
function calculateRoute(shoppingList, worthHop = 3000) {
  const [route, ingredients, craft] = calculateRouteInternal(
    shoppingList,
    worthHop
  );
...
function calculateRouteInternal(shoppingList, worthHop) {
  const ingredientMap = {};
  shoppingList
    .map(([_craft, _raw_, ingredients]) => ingredients)
    .flat()
    .filter((i) => !i.raw)
    .forEach((i) => {
  ...
  const crafting = shoppingList.map(
    ([craftingItem]) =>
      Object.values(craftingItem.itemPrices).length &&
      Object.values(craftingItem.itemPrices)[0].cheapestPPU
  );
  const rawMaterials = shoppingList.map(([_craft, raw, _i]) => raw);

  let possibleWorlds = new Set();
  rawMaterials.forEach((r) =>
    Object.values(r.itemPrices).forEach(({ prices }) =>
      Object.keys(prices).forEach((world) => possibleWorlds.add(world))
    )
  );
  possibleWorlds = [...possibleWorlds];
  const possibleWorldCombinations = new Array(1 << possibleWorlds.length)
    .fill()
    .map((e1, i) => possibleWorlds.filter((e2, j) => i & (1 << j)))
  ...
  routes.forEach((r, id) => {
    ...
    let currPrice = 0,
      bestPrice = 0;
    Object.values(r).forEach((world) =>
      Object.values(world).forEach(
        (item) => (currPrice += item.needed * item.pricePerUnit)
      )
    );
    Object.values(bestRoute).forEach((world) =>
      Object.values(world).forEach(
        (item) => (bestPrice += item.needed * item.pricePerUnit)
      )
    );
...
function generateRoute(worlds, rawMaterials) {
  ...
  rawMaterials.forEach((r) => {
    Object.entries(r.itemPrices).forEach(([id, obj]) => {
      const sortedPrices = Object.entries(obj.prices)
        .filter(([world]) => worlds.includes(world))
        .sort((a, b) => a[1].pricePerUnit - b[1].pricePerUnit);
      ...
      addItem(
        id,
        bestWorld,
        obj.prices[bestWorld].needed,
        obj.prices[bestWorld].pricePerUnit
      );
    });
  });
...
function dumpRoute(route, ingredients, crafting) {
  ...
  Object.values(route).forEach((world) => {
    Object.values(world).forEach(
      (item) => (total += item.pricePerUnit * item.needed)
    );
  });
    ...
    if (c) {
      output +=
        ...
        `[${c.pricePerUnit.toLocaleString(
          'en-US'
        )}G]  [${c.price.toLocaleString('en-US')}G]`;
    }
  });
