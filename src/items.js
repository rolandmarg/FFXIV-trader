...
function get(input) {
  ...
  async function craftProfit(minimumProfit = 0, minimumVelocity = 0) {
    ...
    for (const chunk of itemsChunk) {
      const responses = await Promise.all([ ...chunk.map(i => i.buyCraft()), new Promise(r => setTimeout(r, 1000)) ]);
  ...
  function logProfit() {
    ...
    items.forEach(i => {
      ...
      output += `${('[' + i.craftInfo.price.price.toLocaleString('en-US') + 'G').padEnd(7, ' ')}] `;
      output += `crafting mats [${(i.craftInfo.materialPrices.price.toLocaleString('en-US') + 'G]').padEnd(7, ' ')} `;
      output += `profit ${'[' + i.craftInfo.price.profit + '%]'.padEnd(3, ' ')} `;
      output += `velocity ${'[' + i.craftInfo.price.velocity + '%]'.padEnd(3, ' ')} `;
      output += `available ${'[' + i.craftInfo.price.available + ']'.padEnd(4, ' ')}\n`;
    })
