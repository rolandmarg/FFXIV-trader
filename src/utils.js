...
function normalizeItemInput(i) {
  let normalized;
  if (Array.isArray(i)) {
    normalized = dedupe(i.map(input => isNumeric(input) ?
      { id: input, amount: 1 } :
...
function dumpFile(name, data) {
  return fs.writeFileSync(`./dump/${Date.now()}-${name}`, data, { enconding: 'utf8' });
}
